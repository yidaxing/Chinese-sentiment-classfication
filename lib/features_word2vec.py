...
def create_embedding_weights(model, max_index=0, writeEmbeddingFileName = "./model/embedding_weights.pkl"):
    ...
    for word, value in model.wv.vocab.items():
        ...
        #print("value.index equals "+ value.index +"\n")
...
def get_word2vec_model(data, column, num_features=50, min_word_count=5, num_workers=4, context=5,
                           downsampling=1e-3,
                           model_path="./model/50features_5minwords_5context", remove_stopwords=False):
...
def load_word2vec_model(model_name="50features_5minwords_5context"):
...
def get_avgfeatures_word2vec(data, column, model, num_features = 50, writeFeaturesFileName = "./model/imdb_avgfeatures.pickle" ):
    if(os.path.isfile(writeFeaturesFileName)):
        reviewFeatureVecs = pickle.load(open(writeFeaturesFileName, 'wb'))
    ...
    pickle.dump(reviewFeatureVecs, open(writeFeaturesFileName, 'rb'))
    return reviewFeatureVecs
